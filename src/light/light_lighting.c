...
/*   Updated: 2025/06/06 09:53:55 by jperpct          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void lig_specular_and_difuse(t_lightnig *light, t_computations comp,
                             t_mater mat, t_light luz) {
  light->diffuse = c_scalar_multipl(
      light->efectiv, mat.values.diffuse * light->t_luz_dot_normal);
  light->reflect_dot_eye =
      dot_product(lig_reflect(neg_tuple(light->luztv), comp.norm), comp.eyev);
  if (light->reflect_dot_eye <= EPSILON)
    light->sepcular = c_new(0, 0, 0);
  else {
    light->fact = pow(light->reflect_dot_eye, mat.values.shininess);
    light->sepcular =
        c_scalar_multipl(luz.intenstiy, mat.values.specular * light->fact);
  }
}
...
t_color lig_lighting(t_mater mat, t_light luz, t_computations comp) {
  ...
  light.luztv = normalize(sub_tuples(luz.point, comp.word));
  light.amb_c = c_multipl(
      light.efectiv, c_new(mat.values.amb, mat.values.amb, mat.values.amb));
