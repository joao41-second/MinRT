...
static int	calculate_triangle_intersection(t_ray ray, t_object tri, double *t)
{
	...
	dir_cross_e2 = cross_product(ray.d, tri.u_data.triangle.edge2);
	fuv[0] = dot_product(tri.u_data.triangle.edge1, dir_cross_e2);
	if (fabs(fuv[0]) < EPSILON)
		return (0);
	fuv[0] = 1.0 / fuv[0];
	p1_to_origin = sub_tuples(ray.o, tri.u_data.triangle.p1);
	...
	fuv[2] = fuv[0] * dot_product(ray.d, origin_cross_e1);
