...
/*   Updated: 2025/06/05 13:37:28 by rerodrig         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_plane(char *line, t_minirt *data, int fd, t_list_ **word_objects)
{
	...
	if (!parse_vector(&line, &val[0], range_pos, fd) || \
		...
		return ;
	mat2 = obj_material_init(c_new(val[6] / 255.0, val[7] / 255.0, \
		val[8] / 255.0), obj_init_values_material(0.1, 0.6, 0.3, 100));
	plane = create_plane_parser(create_point(val[0], val[1], val[2]), \
...
void	parse_cylinder(char *line, t_minirt *data, int fd, t_list_ **objects)
{
	double			val[11];
	t_cylinder		cylinder;
	...
	mat1 = obj_material_init(c_new(val[8] / 255.0, val[9] / 255.0, \
		val[10] / 255.0), obj_init_values_material(0.1, 0.6, 0.3, 100));
