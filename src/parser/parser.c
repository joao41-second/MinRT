...
/*   Updated: 2025/06/09 11:45:05 by rerodrig         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	parse_handler(t_token type, char *line, t_parser_ctx *ctx)
{
	if (type == AMBIENT)
		...
		parse_sphere(line, ctx->data, ctx->fd, ctx->word_objects);
	else if (type == PLANE)
		parse_plane(line, ctx->data, ctx->fd, ctx->word_objects);
	else if (type == CYLINDER)
		parse_cylinder(line, ctx->data, ctx->fd, ctx->word_objects);
