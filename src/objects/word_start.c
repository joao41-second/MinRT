...
void start_word(t_minirt *rt_struct) {
  ...
  ray_set_transform_obj(
      obj_sphere2,
      mat_multip(mat_multip(mat_gener_scal(1, 2, 2), mat_gener_trans(0, 1, 1)),
  ...
  ray_set_transform_obj(obj_sphere3, mat_multip(mat_gener_scal(1, 1, 1),
                                                mat_gener_trans(0, 0, 2)));
  ...
  ray_set_transform_obj(obj_plane, mat_multip(mat_gener_scal(1, 1, 1),
                                              mat_gener_trans(0, -4, 0)));
  ft_add_node(obj_plane, &word_objects);

  t_cylinder cyl =
      create_cylinder(create_point(0, 0, 0), create_vector(0, 1, 0), 0.2, 5);
  t_object *obj_cylinder = create_object(&cyl, OBJ_CYLINDER, mat1, NULL);
  obj_cylinder->matiral =
      obj_material_init(c_new(1.0, 1.0, 0), c_new(-1, -1, -1),
  ...
  t_triangle tri1 = create_triangle(
      create_point(2, 1, 0), create_point(2, 0, 0), create_point(2, 0, 5));
  t_object *obj_triangle1 = create_object(&tri1, OBJ_TRIANGLE, mat1, NULL);
  obj_triangle1->matiral =
      obj_material_init(c_new(1.0, 0, 1.0), c_new(-1, -1, -1),
                        obj_init_values_material(0.1, 0.6, 0.9, 100));
  ray_set_transform_obj(obj_triangle1, mat_gener_scal(1, 1, 1));
  //	ft_add_node(obj_triangle1, &word_objects);

  t_triangle tri2 =
      create_triangle(create_point(-1.6, -4.7, 1), create_point(-1.2, -4.9, 1),
                      create_point(-1.6, -4.7, 1.4));
  ...
  // obj_square(obj_create_points( create_point(-1.6, -4.7, 1),
  // create_point(-1.2, -4.9,1), create_point(-1.6,-4.7 ,1.4 ), (t_point)
  // {1,1,2}), word_objects, mat1, mat_gener_scal(2,2,2));
  // obj_square(obj_create_points( (t_point){0,0,0},  (t_point){0,1,0},
  // (t_point){0,1,1}, (t_point) {0,0,1}), word_objects, mat1,
  ...
  obj_open_stl_start(
      ...
      mat_multip(mat_gener_scal(1, 1, 1), mat_gener_trans(0, -2, 0)), mat2);
