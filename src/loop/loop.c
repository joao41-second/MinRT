...
void	loop(t_minirt *rt_struct)
{
	...
	mat_set_trans(&rt_struct->cam_m.calculate.tras, rt_struct->point.x, rt_struct->point.y, rt_struct->point.z);

	mat_set_cp(&rt_struct->cam_m.calculate.restl_temp, &rt_struct->cam_m.tranform_matrix);
	mat_set_multip(&rt_struct->cam_m.tranform_matrix,rt_struct->cam_m.calculate.rot_x,rt_struct->cam_m.calculate.restl_temp);
	mat_set_cp(&rt_struct->cam_m.calculate.restl_temp, &rt_struct->cam_m.tranform_matrix);
	mat_set_multip(&rt_struct->cam_m.tranform_matrix,rt_struct->cam_m.calculate.restl_temp,rt_struct->cam_m.calculate.rot_y);
	mat_set_cp(&rt_struct->cam_m.calculate.restl_temp, &rt_struct->cam_m.tranform_matrix);
	mat_set_multip(&rt_struct->cam_m.tranform_matrix,rt_struct->cam_m.calculate.restl_temp,rt_struct->cam_m.calculate.tras);
